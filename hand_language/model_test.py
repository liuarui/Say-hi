...
def predict(parameters, X):
	...
	with tf.Session() as sess:
		prediction_label, logit = sess.run([c,logits], feed_dict={x: X})
	print("=======================")
...
def img_to_mat(picname):
	im = Image.open("data/new_photo/{}".format(picname))
...
def display_result(mat, prediction):
	...
	font = ImageFont.truetype('/Library/Fonts/Songti.ttc', 80 ,encoding="unic")
	draw.text((100, 100), "输出结果: {}".format(str(prediction)), fill= '#FF0000', font = font)
	im.show()
